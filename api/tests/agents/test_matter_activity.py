...
async def test_matters_rollup_counts_and_newest_status(
    ...
    t_old = await _make_thread(
        db_session, user=user_a, project_id=project.id, last_run_at=_NOW - timedelta(hours=2)
    )
    t_new = await _make_thread(
        db_session, user=user_a, project_id=project.id, last_run_at=_NOW - timedelta(minutes=5)
    )
    await _make_run(
        db_session,
        user=user_a,
...
async def test_matters_excludes_archived_and_sandbox(
    ...
    await _make_project(db_session, owner=user_a, archived=True)
    sandbox = Project(
        owner_id=user_a.id,
        name="Sandbox",
