...
| Admin model-alias CRUD (create / edit / delete) | M1 with known UX gaps | `web/src/routes/lq-ai/admin/models/+page.svelte` + `AliasForm.svelte`; **known issues in M1:** (1) edit modal's Model dropdown shows only the currently-saved model (the autocomplete prop `providerModels` is not populated by the parent — the field remains free-text-editable but autocomplete is missing); (2) the AliasForm renders in dark mode while the surrounding admin page is light, producing low-contrast text. Both tracked as [DE-272](PRD.md#de-272--admin-aliasform-model-dropdown-population--lightdark-consistency); operators can still create / edit / delete aliases in M1, the UX is just rougher than it should be |
