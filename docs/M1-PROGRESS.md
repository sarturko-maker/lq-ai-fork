...
>
> **Last updated:** 2026-05-11 (M1 backend gap-fill — Waves A + B + C closed every frontend-blocking PRD surface the gap audit identified. Wave A: Enhance Prompt endpoint + skill /contents + /inputs + reasoning_visibility preference + EnhancePromptInteraction telemetry (migration 0015). Wave B: /inference/current-tier + /inference/tier-config + real /admin/tier-policy GET+PATCH (replaces D1 stubs; gateway-side update_tier_policy writer) + /admin/usage cost dashboard + /chats/search FTS (migration 0016: tsvectors on chats.title + messages.content). Wave C: RBAC three-role enum users.role with backfill + MutatingUser dependency + PATCH /admin/users/{id}/role with last-admin lockout + WorkProductAttribution table (migration 0017) wired into _persist_assistant_message + GDPR Article 20 export bundle inclusion. +47 api tests → 703 passing; /api/v1 surface 54 → 64 paths. Remaining M1: /metrics, OpenTelemetry, session-timeouts, MFA-mandatory flag, B6 provider remainder, Phase E release-readiness (compliance docs, threat model, CI for SBOM/SLSA/cosign, Helm). Detail in docs/SESSION-HANDOFF-2026-05-11.md.)
