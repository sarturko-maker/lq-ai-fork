...
> **Spike 0 outcome (2026-06-25 — GO):** risks **#1** (author byte-string survives a save + distinct authors coexist and discriminate via `is_ours`), **#2** (personal-info strip OFF by default), **#4** (Collabora-serialised OOXML re-reads cleanly through Adeu; comments survive), and **#11** (zero `<w:ins><w:del>` nesting, including the multi-pass case) are **empirically RESOLVED** on Collabora Office 26.04.1.4 — see `docs/fork/evidence/libreoffice-spike0/`. What remains open: the *narrow* WOPI-name-assignment half of #1 (Collabora's WOPI session stamping the editing user's name — documented + VibeLegalStudio-proven, confirm live in Slice 1/2), the reskin hide surface #5, tracked-MOVES + comment-id stability (part of #4), and the build/infra items #3 and #6–#10.
