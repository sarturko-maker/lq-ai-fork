...
| + | [`fanout-for-document-work-vs-code.md`](fanout-for-document-work-vs-code.md) | **Follow-on (post-E1, 2026-06-28).** *When* to fan out for document work vs code: the **READ/WRITE** + **LOCAL/GLOBAL** split (fan out extraction, single-mind synthesis); trigger = corpus-vs-effective-window **AND** independent-sub-question count, *not* doc count; harness comparison (the fork is **model-autonomous** like Claude Code / deepagents `task`); make it **doctrine-nudge + quota/R4 brake**; eval-gate via a **paired A7-small/A7-large** Track-A extension. Reframes the E1/A7 0/10 fan-out as *the framework working, not a DeepSeek limit*. Input for the Phase-3 strategy/R4 slice. |
