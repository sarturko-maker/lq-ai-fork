...
## State (**Oscar Edition / Agentic Modules milestone OPEN** — **PRIV-8a: ROPA *change verbs* (soft-retire + unlink) — **PR #112 OPEN** (branch `priv-8a-ropa-change-verbs`). The FIRST half of making the Privacy agent able to *change* the register in plain language ("we moved off Mixpanel, we use Hotjar now") — not only append. The agent gains **6 guarded tools**: `retire_processing_activity/_system/_vendor/_transfer` (**soft-retire** — set `retired_at`, never delete, so the change stays **auditable**, the maintainer's explicit requirement) + `unlink_system/_vendor_from_activity` (drop one M:N link). **Reads exclude retired everywhere by default** — the API list/detail/export/summary/data-flow endpoints AND the agent's own `list_*` tools (incl. category usage-counts + transfers of retired parents/recipients); `?include_retired=true` is the audit view; the write/link/tag/transfer verbs refuse a **retired target** so the live register never grows a hidden link. Migration **0063** (nullable `retired_at`+`retirement_reason` on the 4 mutable entities; **up/down verified on throwaway pgvector**). **ADR-F023 ACCEPTED** (soft-retire/supersede; renumbered off the existing F022 data-flow ADR). **Gate:** ruff + mypy clean (168 files); 6 new tool-layer unit tests incl. a `the_swap_replaces_a_vendor` mixpanel→hotjar test + 7 new read-API tests; **fresh-context adversarial review (4 lenses → verify): 0 blockers, 5 should-fixes ALL FIXED** (2 agent read-path divergences in `_list_categories`/`_list_transfers` + export/category test gaps) + the material nits folded (write-side retired-target reject, `_unlink` single-DELETE, `_hide_retired` helper, ADR wording). **NO live-provider run in 8a** (no UI / no new provider behaviour — verified by the containerized integration suite over real Postgres); the LIVE mixpanel→hotjar proof is **PRIV-8b**. On top of **PRIV-7 (PR #111, MERGED — main `789d50d`)** + PRIV-6c/6b/6a/5b/5a/4a/3 (ADR-F019). **RECOMMENDED next pickups:** **PRIV-8b** — the `ropa-maintenance` skill + a `seed_ropa_register` harness helper + the **LIVE mixpanel→hotjar scenario on DeepSeek** + evidence (the payoff the group chat asked for); **PRIV-9** — cockpit UX: chat + register **co-visible** (not the current either/or tabs) + **poll-while-running** so the register updates as the agent works (the side-panel idea); plus carried PRIV-7 follow-ups (the find-or-create **deadlock** HIGH; the `special_category=false`-but-sensitive-category invariant gap; the `ropa-population` binding migration). **Plan:** `docs/fork/plans/PRIV-8-ropa-change-verbs.md`. **NOTE:** still fold the ADR-F021 collaboration workflow output (`wajbzaq82`) into ADR-F021's guest-agent mechanism when Authorization is picked up.)

- **PRIV-8a (PR #112, branch `priv-8a-ropa-change-verbs`) — ROPA change verbs: soft-retire + unlink. Migration 0063 + 6 guarded tools + read filter + ADR-F023. No web.**
