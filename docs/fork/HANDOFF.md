...
## State (**Oscar Edition / Agentic Modules milestone OPEN** — **A4 (conversational-link external intake) is DESIGNED + DEFERRED — ADR-F020 ACCEPTED as the design of record, but BUILD DEFERRED: this machine is never exposed to external traffic (maintainer direction), so the token / unauthenticated-endpoint / rate-limiter / CSP / public-web-page bundle waits for an explicit expose decision. The internal assessment WRITE loop is now HARDENED + PROVEN LIVE (the proof deferred since A2) — see the top bullet. NEXT = maintainer's call: a default-binding skill migration / the carried items below; A4 build resumes ONLY on an explicit expose decision.** PRIV-A3 (the assessment read API + cockpit Assessments tab/detail + the ROPA write-back marker) is DONE — see the top bullet below; PRIV-A2 (validated agent write path + PIA/DPIA skill) and the PRIV-1 review fix (link-table index parity, migration 0065) are DONE too. **P1 = the PRIV-A assessment-automation track** (PIA/DPIA/LIA/TIA) is the biggest genuine gap vs OneTrust/TrustArc (both lead with assessments). Full decomposition in **`docs/fork/plans/PRIV-A-assessment-automation-decomposition.md`** — read it first. **PRIV-A2 SHIPPED** (`app/agents/assessment_tools.py` + the `pia-generation` SKILL.md + the composition grant; see the top bullet). **A3 = assessment read UI + register integration + write-back to ROPA** (mirrors PRIV-3/6b; the Read DTOs + read endpoint land here, deferred from A1) → **A4 = conversational-link external intake, ADR-F020 — THE differentiator (unauthenticated surface; security-heavy; its own ADR)**. **ALSO QUEUED (independent of P1, slot anytime):** **PRIV-6e geographic cross-border transfer map** — `docs/fork/plans/PRIV-6e-geographic-transfer-map.md`: Apache **ECharts** geo-arcs (Apache-2.0, no WebGL, also does Sankey; runner-up d3-geo; amCharts DISQUALIFIED = proprietary linkware). Beats incumbents by showing the **Chapter-V mechanism on arc-click + flagging restricted-without-mechanism** (their geo maps are hosting-location pins, no legal detail). One prereq: `transfer.destination` is free-text → geocode at projection time first (additive), controlled vocabulary later. New dep → ADR + NOTICES (the ADR-F022 precedent). **Research done this session (durable):** gap analysis refreshed vs current OneTrust + TrustArc + shipped-state in **`PRIV-onetrust-to-lqai-functionality-map.md` § Update 2026-06-19** (key findings: both vendors ship node-link AND geographic maps; their 2025 AI — OneTrust Copilot/Privacy-Agent/Winter-'26, TrustArc Arc Dec-2025 — is retrieval/autofill bolt-on, NOT agent-native, so our ADR-F018 validated-write loop + the planned conversational-link intake stay the differentiators; deliberate non-goals reaffirmed: consent/CMP, cookie scanner, data-store connector fleet, licensed regulatory RAG). — *prior:* **Agent-run budget bump: 20 steps/300s → 100 steps/900s — PR #116 MERGED (main `64b6a75`, ADR-F026)** (branch deleted). Cockpit default run budget was a leftover F0-S2 placeholder (`max_steps=20`, wall clock 300s) truncating real multi-change ROPA tasks mid-run with no answer (hit live on DeepSeek flash). Three COUPLED brakes moved together, preserving **in-run wall clock < arq job timeout**: `schemas/agent_runs.py` `max_steps` 20→**100** (ceiling `le=100` kept; R4 cost cap is the money guard, not this); `runner.py` `DEFAULT_WALL_CLOCK_SECONDS` 300→**900**; `agent_run_worker.py` `AGENT_RUN_JOB_TIMEOUT_SECONDS` 420→**1020** (=900+120s slack). Guard test `test_agent_run_timeout_layering` pins the invariant; DB col `server_default=20` left non-operative (route always sets max_steps). **NO schema/migration.** **Gate (all green):** API **2350 pass / 19 skip**, web + gateway green, ruff+mypy clean, fresh-context review SHIP/0 blockers (arq per-fn-timeout precedence verified 3 ways: per-fn 1020 wins, worker default 900 does not cap it). **PROVEN LIVE on DeepSeek flash:** created row `max_steps:100`; a 6-change ROPA run reached **44 settled steps and completed** where the old cap would `cap_exceeded` at 20. Evidence `docs/fork/evidence/agent-run-budget/` (left 3 `VerifyCap …` test rows in the dev register — retire on request). **⚠ Gateway note (operational, UNCOMMITTED, STILL ACTIVE):** `smart`/`fast`/`budget` aliases repointed `minimax/MiniMax-M3 → deepseek/deepseek-v4-flash` on the LOCAL gateway for testing (MiniMax out of quota); runtime config in the `gateway-config` volume, survives restarts; revert recipe in the budget evidence. — *prior:* **PRIV-9b: the changed-row HIGHLIGHT + cross-process live streaming — PR #115 MERGED (main `c4e99f2`)** (branch `priv-9b-changed-row-highlight`). The last half of the group-chat thesis, now real: the user watches the agent change the register AND the changed row lights up. Two coupled pieces, two ADRs. **(1) Change-signal (ADR-F024):** the Privacy agent's guarded ROPA tools record their `(kind,id,verb)` into a run-scoped, B-class `RopaChangeLedger` (`api/app/agents/ropa_changes.py`) after a successful flush — ONLY on a real change (no-ops/rejections record nothing); kinds map to the 3 top-level register tables (link/tag/transfer → the affected activity, + a link also records the system/vendor). The runner drains it onto a **transient `data-ropa-change`** stream frame (`RunStreamPublisher.ropa_changed`); the cockpit lifts the id into a host-hoisted `recentlyChangedIds` set (5 s decay, reset per change) and `RopaRegister` washes the matching row (`--status-completed-wash`, fading; reduced-motion → instant). Model-facing prose unchanged; ids only (audit-safe). **(2) Cross-process streaming (ADR-F025):** runs execute in the arq worker, so F1-S1 had left the SSE stream serving DB-tail only ("live parts simply stop arriving") — the highlight + reasoning ribbon + tool frames couldn't reach the browser. Fixed with a publish-only `RedisStreamBroker` in the worker (fans live parts onto Redis pub/sub; the SAME `RunStreamPublisher` reused) + a reference-counted `RedisStreamBridge` in the api that republishes them into the **existing** process-local broker — SSE endpoint + runner + publisher **UNCHANGED**. **PROVEN LIVE on DeepSeek** (`deepseek` alias — `smart`→MiniMax is out of quota): 3 use-cases (add system / retire system / add vendor) each completed and emitted the right `data-ropa-change` frame, with the full live stream now crossing worker→Redis→api→browser. **Browser visual:** `priv-9b-highlight.cy.ts` (headed, real-frame) — Hotjar row washed green beside the run-locked chat, light+dark. **NO schema/migration.** **Gate:** API full suite **2346 passed / 19 skipped** (+ `test_ropa_changes`, `test_agent_stream_redis` fake-redis round-trip, change-signal recording tests, `ropa_changed` publisher tests); ruff+mypy clean; web `npm run check` 0 err + **893 vitest**; Cypress highlight spec pass. Evidence + FINDINGS `docs/fork/evidence/priv-9b/`. **Dev-only:** `ropa-maintenance`+`ropa-population` bound to Privacy IN THE DEV DB for the live test (production binding migration still Backlog); the 3 live runs left reversible changes in the dev register. **Plan:** `docs/fork/plans/PRIV-9-cockpit-live-register.md`. — *prior:* **PRIV-9a: watch the agent change the register — PR #114 MERGED (main `a9a9a77`)** (branch `priv-9a-covisible-runlock-liverefresh`). The PAYOFF UX of the group-chat thesis, now visible. In a Privacy matter the conversation and the deployment-global **ROPA register sit side by side** (resizable `Resizable.PaneGroup`; the old one-at-a-time toggle is the narrow fallback — and there the conversation stays **mounted (hidden)** so it still live-updates). While the agent works the composer **locks to a single Stop button** (input/matter-select/attach **removed from the DOM**, not merely disabled) that calls the **real backend cancel** (`POST /agents/runs/{id}/cancel`, ADR-F009) and re-syncs by polling to the settled `cancelled` row (ADR-F004 — the polled row decides). The register **re-reads live**: a quiet `reload()` (**no skeleton flicker**) on a 2 s `runActive`-gated self-rescheduling poll with a `pollGeneration` guard, + one reconcile on settle (host bumps `reloadKey`). **Read-only throughout (ADR-F019): the register UI only ever GETs; the agent stays the sole, audited writer.** Frontend-only — **NO schema, NO migration, NO new ADR**. Files: `web/.../cockpit/ConversationHost.svelte` (co-visible split + toggle fallback + `runActive`/`registerReloadKey`), `web/.../components/agents/ConversationPanel.svelte` (run-lock Stop + `cancelCurrentRun` + `pendingRunId`), `web/.../components/ropa/RopaRegister.svelte` (props `runActive`/`reloadKey`, quiet poll), `web/.../api/agents.ts` (`cancelRun`), `web/.../agents/helpers.ts` (pure `agentWorking`/`cancellableRunId`). **Gate:** `npm run check` 0 errors; full vitest **890 passed** (+ `agentWorking`/`cancellableRunId`/`cancelRun` tests); **two headed Electron specs pass** (`priv-9a-covisible` + `priv-9a-runlock`, deterministic intercepts — no LLM, no register writes); **measured commit→row-visible ≈ 1.1 s** (`docs/fork/evidence/priv-9a/time-to-visible.json`); **fresh-context adversarial review (4 lenses → refute-by-default): 14 raised, 9 confirmed (5 should-fix + 4 nits); all 5 should-fixes FIXED** (poll double-loop race → `pollGeneration`; dead `submitting`/`'Starting…'` → removed; Stop disabled in the create-run window → `pendingRunId` seed; no live-refresh in the 720–1168 px toggle band → keep panel mounted; missing run-lock/no-flicker test → added). Evidence + honest FINDINGS (resize-remount nit; narrow-dashboard cosmetics; a pre-existing "Ask the Commercial agent" label) in `docs/fork/evidence/priv-9a/`. **RECOMMENDED next pickup: PRIV-9b** — the changed-row **highlight**: the agent→register change-signal (ROPA tool results return the entity id; a structured `changed:{kind,id,verb}` on the tool-output stream frame; client lifts touched ids; register flashes the matched rows) so the user sees *which* row changed — crosses the agent→UI module boundary → **ADR-F024**. Also pending: ship `ropa-maintenance`+`ropa-population` via a **binding migration**; PRIV-7 follow-ups (find-or-create **deadlock** HIGH; `special_category=false`-but-sensitive gap). **Plan:** `docs/fork/plans/PRIV-9-cockpit-live-register.md`. **NOTE:** still fold ADR-F021 collaboration output (`wajbzaq82`) into ADR-F021 when Authorization is picked up. — *prior:* **PRIV-8b: the LIVE mixpanel→hotjar proof — PR #113 MERGED (main `cab7ec3`)** (branch `priv-8b-live-swap-skill`). The PAYOFF of the group-chat thesis: a lawyer says, in plain language, *"we moved off Mixpanel, we use Hotjar now"* and the Privacy agent **composes the PRIV-8a change verbs into a coherent, audited swap** — Hotjar added + linked, Mixpanel unlinked **and soft-retired** (kept on record), then reports exactly what it changed. **Proven live on DeepSeek V4** (`max_steps=80`, 3 arms): **both skilled arms → coherent** (`deepseek`+skill and `deepseek-pro`+skill ran the full add→link→unlink→retire→confirm→report and completed); the **no-skill baseline → "lists BOTH"** (retired the Mixpanel system but left the vendor live+linked, register shows `[Hotjar, Mixpanel]`) — so the new **`ropa-maintenance` skill is load-bearing**. NOTHING destroyed (Mixpanel survives soft-retired, auditable). No `app/` code touched — this is the skill + harness + evidence: **`skills/ropa-maintenance/SKILL.md`** (swap method; retire=company-wide soft, unlink=one activity; never leave both); **`ropa_eval.py`** `seed_ropa_register` (plants the pre-existing Mixpanel register) + `retired` surfaced in `snapshot_register` + pure **`evaluate_swap`** scorer (live-register view; gates `coherent` on the activity itself being live; flags duplicate names); **`test_ropa_update_scenario.py`** (provider-gated live, asserts terminal+model-turn only per ADR-F015) + **`test_ropa_update_eval.py`** (8 non-provider CI tests). Evidence + honest FINDINGS in `docs/fork/evidence/priv-8/`. **Gate:** ruff (CI root config) + mypy clean; full api non-provider suite green (2327→2329 with the +2 new eval tests; +8 total); live run on dev gateway→DeepSeek with throwaway test DBs, **dev register verified pristine (0 leaked rows)**; **fresh-context adversarial review (4 lenses → refute-by-default): 8 findings, 4 confirmed (1 should-fix + 3 nits) ALL FIXED** — the should-fix: `evaluate_swap` over-reported `coherent` if the agent retired the whole *activity* (now gated + unit-tested); nits: duplicate-name collapse (prefer-live + flag), skill reason-length guidance (~200→real ~1000), an unused `SeededRegister` field trimmed. Skill bound **test-only** (PRIV-7 precedent); default-binding migration follows. On top of **PRIV-8a (PR #112, MERGED — main `0702e79`)** [soft-retire+unlink verbs, migration 0063, ADR-F023] + **PRIV-7 (PR #111, MERGED — `789d50d`)** + PRIV-6c…3 (ADR-F019). **RECOMMENDED next pickups:** **PRIV-9** — cockpit UX: chat + register **co-visible** (not the current either/or tabs) + **poll-while-running** so the register updates as the agent works (the side-panel idea the group chat asked for, now that the capability is proven); ship `ropa-maintenance`+`ropa-population` via a **binding migration**; carried PRIV-7 follow-ups (find-or-create **deadlock** HIGH; the `special_category=false`-but-sensitive invariant gap). **Plan:** `docs/fork/plans/PRIV-8-ropa-change-verbs.md`. **NOTE:** still fold the ADR-F021 collaboration workflow output (`wajbzaq82`) into ADR-F021's guest-agent mechanism when Authorization is picked up.)

- **▶ PICK UP EXACTLY HERE: SWITCHING PRACTICE AREAS → COMMERCIAL.** Per maintainer direction the focus
  moves off Privacy to the **Commercial** practice area (a completely different area). **Privacy is PARKED, not
  abandoned** — its full outstanding TODO is consolidated in **`docs/fork/plans/PRIV-BACKLOG.md`** (read that
  to resume Privacy). One-liners of what's parked: a default-binding skill migration (Tier-1, ready); the
  PRIV-7 find-or-create **deadlock** (HIGH); the `special_category=false`-but-sensitive gap; PRIV-6e geographic
  transfer map; PRIV-6d legal-entity scope; the substantive legal-quality track; and **PRIV-A4 (deferred —
  resumes ONLY on an explicit decision to expose this machine, ADR-F020)**. **For COMMERCIAL:** the substrate
  already supports it (Commercial is a configured practice area with its area agent + the
  `document-researcher` subagent, UX-B-1/4; `seed_commercial_matter` + the scenario harness exist). Await the
  maintainer's specific Commercial goal; there is no Commercial decomposition plan yet — expect explore →
  written plan first (CLAUDE.md), mirroring how the Privacy module was built.

- **A4 DESIGN + DEFERRAL + assessment-loop HARDENING (this session, branch `priv-a4-design-deferral-and-loop-hardening`) — docs + tests + evidence; NO `app/` change, NO migration; ADR-F020 (accepted, build-deferred).**
  Two coupled outcomes. **(1) PRIV-A4 designed, then DEFERRED on maintainer direction (no machine exposure).**
  ADR-F020 (`docs/adr/F020-conversational-link-external-assessment-intake.md`) records the full design — the
  opaque-token+DB-row model, the per-assessment firm-created shell, the scoped `{add_risk}` agent
  (assessment_id closure-injected; composition short-circuit), deferred-review (the agent can't complete; the
  firm completes in the A3 cockpit), the unauthenticated-surface security envelope, and the token-bearer authz
  posture — **ACCEPTED as the design of record but BUILD DEFERRED**: the token, the unauthenticated endpoint,
  the rate-limiter / strict-CSP / serving infra, and the public web page all wait for an explicit expose
  decision (building them now = consumer-less code + an exposure surface we don't want; "do the plumbing and
  STOP"). Sub-track plan bannered ⛔ DEFERRED:
  `docs/fork/plans/PRIV-A4-conversational-link-intake-decomposition.md`. Grounded by a 6-subsystem mapping
  workflow + a 20-agent adversarial ADR review (7 should-fixes folded: token-bearer authz posture stated;
  rate-limiter/CSP scope tightened; refresh-token reuse + cost-cap spec pinned). **(2) The internal assessment
  WRITE loop is PROVEN LIVE** — the proof deferred since A2 (A3 only verified the *read* side on a *seeded*
  DPIA). New `tests/agents/scenarios/assessment_eval.py` (snapshot/score/cleanup — the `ropa_eval` sibling) +
  `test_assessment_build_scenario.py` (provider-gated, 3 arms flash/flash+skill/pro+skill; synthetic note +
  seeded register; throwaway DB; ADR-F015 asserts + the F027 integrity assert) + pure CI scorer tests
  `test_assessment_eval.py` (7). **LIVE on DeepSeek (dev gateway, throwaway DB, dev register UNTOUCHED):** all 3
  arms built a defensible, **completed** DPIA, every risk with a documented design-tied mitigation,
  **F027 completion-integrity held on every arm**. Finding: even the no-skill baseline builds a valid DPIA but
  spawned a stray 2nd draft + capped before reporting; **both skilled arms produced one clean, completed, linked
  DPIA and reported back** — the `pia-generation` skill earns its place via FOCUS, not necessity. Raised the
  scenario `max_steps` 80→100 (the ADR-F026 cockpit ceiling) for report-back headroom. Evidence + FINDINGS:
  `docs/fork/evidence/priv-a2/`. **Gate (all green):** ruff+mypy clean; **full API suite 2428 passed / 22
  skipped** on a throwaway pgvector (+7 pure scorer tests, +3 provider-skip); fresh-context 3-lens adversarial
  review (correctness + safety CLEAN; 1 simplification folded). **No exposure: everything ran locally against a
  throwaway DB + the local gateway.** ⚠ Gateway aliases `smart`/`fast`/`budget` still repointed to
  `deepseek-v4-flash` (uncommitted, local; MiniMax out of quota).

- **PRIV-A3 (PR #121, branch `priv-a3-assessment-read-ui`) — assessment read API + cockpit Assessments tab/detail + ROPA write-back. API + web; NO migration; ADR-F018/F019/F027 (no new ADR).**
  The read half of the assessment track, mirroring PRIV-3/6b on the A1 spine + A2 write path; **read-only
  throughout (ADR-F019 — the Privacy agent stays the sole audited writer; the UI only GETs).** **Backend:**
  the `*Read` DTOs deferred from A1 — `AssessmentRead`/`RiskRead`/`AssessmentActivityRef` in
  `app/schemas/assessment.py` (+ `AssessmentSummary`, the compact write-back projection); **`app/api/assessments.py`**
  `GET /ropa/assessments` (+ `/{id}`) under the same shared-read `_active` mount as the ROPA reads (404 =
  genuinely missing id; linked activities are **live-only** via `with_loader_criteria`, parity with the agent
  list + the ROPA reads). Route count **140→142** (`test_openapi`); `IMPLEMENTED_ROUTES` + `_PARAM_VALUES`
  updated (`test_endpoints`). **Write-back = a read-only PROJECTION, not a persisted field / second writer**
  (the design call — no new ADR; it implements F019): a `ProcessingActivity.assessments` **viewonly** ORM
  relationship (string-ref, NO migration; the reverse selectinload is index-backed by `ix_apa_processing_activity_id`
  from migration 0065 — the PRIV-1 fix pays off here) surfaced as `ProcessingActivityRead.assessments`, loaded
  on both activity endpoints **and** `_load_register`. The Article 30 **export deliberately EXCLUDES** the marker
  (the regulatory deliverable's shape must not drift — `model_dump_json(exclude=…)`; regression-tested).
  **Frontend (F013, semantic tokens, read-only):** an **Assessments** register tab (list) + `AssessmentDetail`
  (risks table + linked activities) + a **"DPIA on file"** marker (badge on the activities table + activity-detail
  hero, plus an Assessments chips section deep-linking to the assessment). Pure `dpiaOnFile()` helper
  (type==dpia && status==completed). **Gate (all green):** **API full suite 2421 passed / 19 skipped** on a
  throwaway pgvector (+ `test_assessment_read.py`: list/detail/404/401, source_project_id-off-the-wire,
  retired-activity-hidden, the write-back projection, shared-read across users; + export regression); ruff+mypy
  clean. Web: svelte-check 0 err; **vitest 900 passed** (+ dpiaOnFile/label/tab-order); prettier+eslint clean.
  **PROVEN LIVE on the dev stack** (api+workers+web rebuilt — auto-applied the throwaway-verified 0064+0065; dev
  DB now at **0065**): `GET /ropa/assessments` 401 unauth / 200 authed; a seeded completed DPIA rendered in the
  list + detail (risks, linked activity); the activity carried the **DPIA-on-file** projection; 404 on missing
  id; `source_project_id` confirmed off the wire; **dev register cleaned back to pristine (0 leaked rows)**.
  **Headed Cypress** (`priv-a3-assessments.cy.ts`, 2 specs green) — Assessments list (light+dark × wide+narrow),
  detail, the DPIA-on-file activities view + write-back chips; evidence in `docs/fork/evidence/priv-a3/`.
  **Fresh-context adversarial review (6 lenses × 3 refute-by-default skeptics, 21 agents): 5 raised, 3 confirmed
  (2 should-fix + 1 nit), 2 refuted — ALL 3 folded:** (1) the write-back field had silently expanded the Article
  30 JSON export → excluded + regression test; (2) `listAssessments()` joined the register `load()` Promise.all
  but the priv-9a/9b isolation specs didn't stub `/ropa/assessments` (latent flake) → intercept added to both
  (re-run GREEN); (3) dropped the unused `AssessmentSummary.risk_rating` (the marker is type+status; the rating
  lives on the assessment's own read).

- **PRIV-A2 (branch `priv-a2-assessment-write-path`) — validated assessment agent write path + PIA/DPIA skill. API + skill; NO endpoint, NO schema/migration; ADR-F018/F027 (no new ADR).**
  Mirrors PRIV-2 (`ropa_tools`) on the A1 spine: the Privacy Deep Agent now builds and completes privacy
  assessments through guarded, code-validated tools. **`app/agents/assessment_tools.py`** (`build_assessment_tools`):
  `propose_assessment` / `add_risk` (validate against `AssessmentInput`/`RiskInput`, commit or reject-and-retry);
  **`complete_assessment`** — the transition to `completed`, gated by the headline cross-row invariant
  (ADR-F027: a DPIA, or any `high`-rated assessment, can't complete without ≥1 risk carrying a non-blank
  mitigation) via the pure `validate_assessment_completable` guard — **this verb is added beyond the plan's
  4-tool sketch** because it (and create-completed) are the only live paths that reach `completed`, so it's
  where the cross-row guard must run; `link_assessment_to_activity` (M:N to a ROPA activity; refuses
  unknown/retired; idempotent); `list_assessments`. All via `guarded_dispatch` (R5/R6 + one audit row,
  counts/types/IDs only); `source_project_id` closure-injected provenance (ADR-F019); model-facing signatures
  A-class only. **`composition.py`** grants the assessment tools to a Privacy matter alongside the ROPA tools
  (same area-key point); they reach activity IDs via the ROPA `list_processing_activities`, so no duplicate
  list; **no change ledger** (the live-highlight consumer is the A3 read UI). **`skills/pia-generation/SKILL.md`**
  (port-and-improve PIA/DPIA/LIA/TIA method: scope→score concrete risks to individuals→specific design-tied
  mitigations→complete only when the high-risk rule is satisfied); **bound test-only** (PRIV-7 precedent;
  default-binding migration later). **Gate (all green):** ruff+mypy clean from repo root; **FULL API suite
  2409 passed / 19 skipped / 0 failed** on a throwaway pgvector (+ the new `test_assessment_tools` covering
  both directions of the headline rule, link/retired-refusal/idempotency, audit-without-values, A-class
  signatures, grant-set drift, and a real-loop propose→risk→complete; + the composition grant test + the
  `pia-generation` loader test); skill YAML parses. **Live verification:** deferred to A3 when there is a
  user-facing read surface (A2 is agent-internal; the real-loop test exercises the full propose→complete loop
  with a scripted model). **Fresh-context adversarial review (6 lenses × refute-by-default, 44 agents): 13
  raised, 9 confirmed (1 should-fix + 8 nits), 4 refuted — invariant fidelity + security confirmed CLEAN (no
  bypass to `completed`).** All substantive findings folded: 3 added tests (the should-fix = the
  stored-rating high-risk completion path; create-completed high non-DPIA; re-rate-completed re-validation) +
  2 docstring fixes (the `complete_assessment` re-rate behaviour; the test-module e2e note). The one deferred
  nit (the DB-CHECK-backstop code comment is untested at the tool layer) matches the ROPA precedent verbatim
  and A1's `test_assessment.py` already proves those CHECKs reject at the model level.
...
- **PRIV-A1 (PR #118, branch `priv-a1-assessment-domain-spine`) — assessment domain spine + code validation. API-only (migration `0064`); NO agent, NO UI; ADR-F027.**
  ...
  open|mitigated|accepted, `extra="forbid"`), `app/models/assessment.py` (`Assessment` + `Risk` +
  `assessment_processing_activities` M:N, CHECK mirrors), migration `0064` (next head). **Headline ADR-F018/F027
  invariant** (the assessment analogue of `special_category⇔art9_condition`): *a DPIA — or any `risk_rating='high'`
  assessment — can't be `completed` without ≥1 `risk` carrying a non-blank `mitigation`*, split by what each
  layer can express: the WITHIN-ROW half (`completed ⇒ risk_rating present`) is a Pydantic `model_validator`
  **+ a DB CHECK** (`chk_assessments_completed_requires_rating`); the CROSS-ROW half (completed high-risk ⇒ a
  documented mitigation) is the pure app-layer `validate_assessment_completable` guard (NOT a DB trigger — the
  single-writer assumption, called out in ADR-F027). `risks.assessment_id` CASCADE; link table CASCADE both
  ends; `source_project_id` provenance-only SET NULL (deployment-global, ADR-F019). **Fixed the ROPA carried
  debt:** `updated_at` carries `onupdate`. **Deliberately deferred** (matches PRIV-1→PRIV-3): Read DTOs +
  read endpoint → A3; soft-retire → later (ROPA added it in PRIV-8a). **Gate (all green):** ruff + mypy clean
  from repo root; migration **up/down/up clean on a throwaway pgvector** (never the dev DB); full API suite
  **2378 passed / 18 skipped / 0 failed** (+28 new assessment tests; no route-coverage/OpenAPI contract tripped
  — A1 adds no endpoints); **fresh-context adversarial review (5 lenses — correctness/security/migration/
  simplification/precedent — each finding refute-by-default verified): 2 nits raised, 0 confirmed** (both
  refuted live: the `onupdate` "server-side" wording is defensible + the single-writer caveat is in ADR-F027;
  the bottom-of-file import is correct + documented, no real circular import). **Live verification N/A** — no
  endpoint/agent/UI, so no user-facing behavior changes this slice. **NOT yet wired:** no agent tools, no
  endpoint, no skill — all PRIV-A2/A3.

- **Agent-run budget (PR #116 MERGED — main `64b6a75`, branch deleted) — cockpit run budget 20 steps/300s → 100 steps/900s. API-only; NO schema/migration; ADR-F026.**
  The default `max_steps=20` (F0-S2 placeholder, never overridden by the cockpit) truncated real ROPA work
  with no answer. Three coupled brakes raised together so the **clean in-run cap fires before arq's hard kill**:
  `AgentRunCreate.max_steps` 20→100 (`schemas/agent_runs.py`; `le=100` kept), `DEFAULT_WALL_CLOCK_SECONDS`
  300→900 (`agents/runner.py`), `AGENT_RUN_JOB_TIMEOUT_SECONDS` 420→1020 (`workers/agent_run_worker.py`,
  =900+120s slack). Invariant `in-run wall clock < arq timeout` guarded by `test_agent_run_timeout_layering`;
  R4 cost cap remains the money guard. DB col `server_default=20` left (non-operative — route always sets it).
  **Gate (all green):** ruff+mypy clean; full API suite **2350 pass / 19 skip**; web + gateway green;
  fresh-context review SHIP/0 blockers (arq per-fn-timeout precedence verified 3 ways).
  **LIVE on DeepSeek flash** — created row `max_steps:100`; a 6-change ROPA run hit **44 steps and completed**
  (would've capped at 20). Evidence `docs/fork/evidence/agent-run-budget/`. **⚠ Operational (UNCOMMITTED, still
  active):** repointed `smart`/`fast`/`budget` aliases → `deepseek-v4-flash` on the local gateway (MiniMax out
  of quota; survives restarts via the `gateway-config` volume).

- **PRIV-9a (PR #114, branch `priv-9a-covisible-runlock-liverefresh`) — cockpit: co-visible chat + register, run-lock, live-refresh. Frontend-only; NO schema/migration/ADR.**
  The group-chat payoff made visible: chat and the ROPA register **side by side** (resizable; toggle fallback
  when narrow, where the conversation stays mounted-hidden so it still live-updates); the composer **collapses
  to a single Stop** while the agent works (input removed from the DOM; real backend cancel + poll re-sync,
  ADR-F009/F004); the register **polls live** (quiet `reload()` — no skeleton flicker — gated on `runActive`
  via a `pollGeneration`-guarded 2 s loop, + reconcile-on-settle). **Read-only (ADR-F019): the UI only GETs.**
  New pure helpers `agentWorking`/`cancellableRunId` + `agentsApi.cancelRun`. **Verify:** `npm run check` 0
  errors; vitest 890 pass; **2 headed Electron specs** (`priv-9a-covisible` layout, `priv-9a-runlock` run-lock
  + live row + no-flicker + Stop→cancel→re-enable, deterministic intercepts); **commit→visible ≈ 1.1 s**
  measured. **Adversarial review: 9/14 confirmed, all 5 should-fixes fixed** (poll double-loop, dead
  `submitting`, Stop-window `pendingRunId`, toggle-band live-refresh, run-lock test). Evidence
  `docs/fork/evidence/priv-9a/` (incl. the hero `priv-9a-live-row-appeared.png`). **NOT in 9a:** the changed-row
  **highlight** + the agent→register change-signal (→ **PRIV-9b**, ADR-F024).

- **PRIV-8b (PR #113, MERGED — main `cab7ec3`) — live mixpanel→hotjar proof + `ropa-maintenance` skill + `seed_ropa_register`/`evaluate_swap`. Tests + 1 skill + evidence; NO `app/` code, NO migration.**
