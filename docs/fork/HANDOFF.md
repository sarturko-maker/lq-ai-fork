...
- **Agent-run budget (branch `agent-run-budget-100-900`, PR pending) — raise the cockpit run budget 20 steps/300s → 100 steps/900s. API-only; NO schema/migration; ADR-F026.**
  ...
  **Verify:** ruff+mypy clean; targeted agent-run tests 64 pass (+ the new invariant test + default→100);
  **LIVE on DeepSeek flash** — created row `max_steps:100`; a 6-change ROPA run hit **44 steps and completed**
  (would've capped at 20). Evidence `docs/fork/evidence/agent-run-budget/`. **PROVEN-ONLY note:** repointed
  `smart`/`fast`/`budget` aliases → `deepseek-v4-flash` (runtime/uncommitted; MiniMax out of quota).
