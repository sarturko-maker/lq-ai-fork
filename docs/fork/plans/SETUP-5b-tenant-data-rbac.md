...
- Viewer enforcement on the legacy Autonomous Layer (`/autonomous/*` frozen, allowlisted) → backlog.
- `chat_receipts.py:103` operator cross-user READ bypass + its 403 existence-leak (recon §6 gap) →
  backlog follow-up seam-fix.
...
- `api/app/api/dependencies.py` — `operator` added to `_MUTATING_ROLES`; docstring; new
  `tenant_admin_visibility(user)` helper (ADR-F064 D2 seam).
...
### mutating → MutatingUser (52; D1 swap)
...
same class and were swapped for completeness.
...
### allow (38; justified exceptions, keep own gate)
...
| `/autonomous/*` | 16 | legacy Autonomous Layer (M4), per-user isolated + per-user opt-in gate; viewer enforcement deferred (CLAUDE.md freezes legacy executors). |

## D2 seams (13; `user.is_admin` → `tenant_admin_visibility(user)`)
...
- `tests/test_mutation_rbac.py` (drift guard: 52 gated / 171 paths / 124 mutating / allowlist minimal;
  viewer-403 + member-passes parametrized across routers; operator cross-user 404 + org-admin sees-all
  regression on tabular/playbooks).
- Regression: `test_tabular_endpoints`, `test_playbook_*`, `test_wave_c`, `test_endpoints`,
  `test_openapi` green.
