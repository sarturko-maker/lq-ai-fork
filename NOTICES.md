...
| `web/src/lib/components/ui/**` + `web/src/lib/utils.ts` vendored shadcn-svelte component source (F1-S2, ADR-F006) | https://github.com/huntabyte/shadcn-svelte — generated by `shadcn-svelte@1.3.0` CLI (`init` preset `b0` nova/neutral + `add` of 11 components), committed verbatim; the CLI is NEVER run in CI — re-running `update`/`add` is an explicit, reviewed change | MIT (upstream `LICENSE.md`) | Vendored source is exempt from house lint/format (`.eslintignore`/`.prettierignore`) to stay diffable against upstream. New devDependencies introduced (build-time; the runtime image carries only the built bundle): `shadcn-svelte` MIT (its `dist/tailwind.css` compiles into the bundle), `bits-ui` MIT, `paneforge` MIT, `@lucide/svelte` ISC (pinned ^1.17.0 — registry items pin an older major; do not let `add` runs downgrade), `tailwind-variants` MIT, `tailwind-merge` MIT, `tw-animate-css` MIT, `clsx` MIT, `@internationalized/date` Apache-2.0. Transitives: `@floating-ui/{core,dom,utils}` MIT, `tabbable` MIT, `dequal` MIT, `lz-string` MIT, `style-to-object` + `inline-style-parser` MIT, `@swc/helpers` Apache-2.0, `node-fetch-native` MIT (CLI-only), `runed` + `svelte-toolbelt` MIT (no `license` field in some published package.json versions — MIT per the LICENSE file shipped in each tarball, © Hunter Johnston). |
